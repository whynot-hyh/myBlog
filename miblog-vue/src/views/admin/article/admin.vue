<template>
    <div>
        ...
        <div class="whitebg bloglist">
            <div>
                <el-form :inline="true" ref="requestType" :model="pageEntity" label-width="80px">
                    <el-form-item>
                        <el-input v-model="pageEntity.search" placeholder="按标题查询"></el-input>
                    </el-form-item>
                    <el-form-item>
...
`
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button @click="setting(scope.row)"  size="small" type="info" icon="el-icon-setting" title="修改权限"></el-button>
            ...
            <el-dialog :visible.sync="EditUserVisble" v-if="EditUserVisble" :close-on-click-modal="false" title="修改员工">
                <setting @after="after" :data="userData" @hideDialog="hidden"></setting>
            </el-dialog>
        </div>
    </div>
</template>
...
<script>
    ...
    import {search,del} from "../../../api/article";
    import token from "../../../../public/js/token";
    import axiosIstance from "../../../utils/request";
    import setting from "../../../components/admin/article/setting";
    export default {
        name: "index",
        components:{setting},
        data(){
        ...
        methods:{
            ...
            search(){
                search(this.pageEntity).then(data=>{
                    console.log('aaa')
                    console.log(data)
            ...
            list(page){
                axiosIstance.post('article/list',page).then(data=>{
                    console.log('aaa')
                    console.log(data)
                    this.articleTable=data.data.list
                    this.total=data.data.total
